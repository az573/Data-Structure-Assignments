...
List* initializeList(){
    ...
    myList->capacity = INITIAL_CAPACITY;
	 myList->data = (Entry**)malloc(sizeof(Entry*) * INITIAL_CAPACITY);
    myList->size = 0;
...
void doubleCapacity(List* myList){
    ...
    myList->capacity *= 2;
    myList->data = (Entry**)realloc(myList->data, myList->capacity * sizeof(Entry*));

}

void halveCapacity(List* myList){

    if (myList->capacity > INITIAL_CAPACITY && myList->size < myList->capacity / 2) {
...
void deleteFromPosition(List* myList, int position){
    ...
    if (position < 0 || position >= myList->size) {
        ...
        printf("Not a valid number of position\n");
        return;
